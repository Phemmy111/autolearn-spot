...
export async function GET(
  ...
  try {
    ...
    if (error) {
      console.error('Error fetching questions for quiz:', id, error)
    ...
    if (error.message?.includes('Unauthorized')) {
      console.error('Unauthorized attempt to fetch questions')
...
export async function POST(
  ...
  try {
    ...
    if (error) {
      console.error('Error creating question for quiz:', id, error)
      return NextResponse.json({ error: error.message }, { status: 500 })
    }
    ...
    if (error.message?.includes('Unauthorized')) {
      console.error('Unauthorized attempt to create question')
      return NextResponse.json({ error: 'Unauthorized: Admin access required' }, { status: 403 })
    }
