...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log('[POST /api/repair-partner-auth] Repairing auth for partner:', partnerId);
    ...
    console.log('[POST /api/repair-partner-auth] Partner found:', partner.email, 'Type:', partner.partner_type);
    ...
    if (existingLinkId) {
      ...
      if (updateError) {
        console.error('[POST /api/repair-partner-auth] Failed to update auth record:', updateError);
        return NextResponse.json({ error: 'Failed to update auth record', details: updateError.message }, { status: 500 });
      }

      console.log('[POST /api/repair-partner-auth] Updated existing auth record');
    } else {
      ...
      console.log('[POST /api/repair-partner-auth] Creating new auth record in table:', authTableName);
      ...
      if (createError) {
        console.error('[POST /api/repair-partner-auth] Failed to create auth record:', createError);
        return NextResponse.json({ error: 'Failed to create auth record', details: createError.message }, { status: 500 });
      }

      console.log('[POST /api/repair-partner-auth] Created new auth record:', authRecord.id);
      ...
      if (linkError) {
        console.error('[POST /api/repair-partner-auth] Failed to link auth record:', linkError);
        return NextResponse.json({ error: 'Failed to link auth record', details: linkError.message }, { status: 500 });
      }

      console.log('[POST /api/repair-partner-auth] Linked auth record to partner');
    }
    ...
    console.error('[POST /api/repair-partner-auth] Error:', error);
