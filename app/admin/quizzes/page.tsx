      ...
      <div className="container mx-auto px-4 py-12">
        <div className="flex items-center justify-between mb-12">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="font-heading text-4xl font-bold text-neutral-900 mb-4">Manage Quizzes</h1>
              <p className="font-mono text-sm text-neutral-600">Create and manage weekly quizzes</p>
            </div>
          </div>
          <div className="flex gap-3">
            <Link
              href="/admin/quizzes/generate"
              className="flex items-center gap-2 bg-[#10b981]/10 text-[#10b981] font-bold uppercase tracking-wider font-mono px-6 py-2 rounded hover:bg-[#10b981]/20 transition-colors text-sm border border-[#10b981]/30"
            >
        ...
        {quizzes && quizzes.length === 0 ? (
          <div className="text-center py-12 border border-neutral-200 bg-white rounded-xl">
            <p className="font-mono text-sm text-neutral-600">No quizzes yet. Create your first quiz!</p>
          </div>
        ) : (
          <div className="space-y-4">
            {quizzes?.map((quiz: { id: string; title: string; week_number: number; is_active: boolean; passing_score: number }) => (
              <div
                key={quiz.id}
                className="border border-neutral-200 bg-white p-6 rounded-xl hover:border-[#10b981]/30 transition-all"
              >
                <div className="flex items-start justify-between mb-4">
                  <div className="flex-1">
                    ...
                    {quiz.description && (
                      <p className="font-mono text-sm text-neutral-600 mb-4">{quiz.description}</p>
                    )}
                  </div>
                </div>

                <div className="flex flex-wrap gap-4 mb-4 font-mono text-xs text-neutral-500">
                  {quiz.time_limit && (
                ...
                <div className="flex items-center gap-2">
                  <Link
                    ...
                    className="flex items-center gap-2 text-[#10b981] hover:text-neutral-900 font-mono text-xs uppercase tracking-wider transition-colors"
                  >
                  ...
                  <Link
                    ...
                    target="_blank"
                    className="flex items-center gap-2 text-neutral-600 hover:text-neutral-900 font-mono text-xs uppercase tracking-wider transition-colors"
                  >
