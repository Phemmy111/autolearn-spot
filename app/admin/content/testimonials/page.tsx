        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-4">
              ...
              <div>
                <h1 className="text-xl font-bold text-brand-text">Testimonials</h1>
                <p className="text-sm text-brand-text/70">Manage student testimonials (WhatsApp screenshots)</p>
              </div>
            </div>
            <button
              onClick={() => {
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {items.map((item) => (
            <div key={item.id} className="border border-brand-border bg-[var(--card)] brightness-95/50 backdrop-blur-xl rounded-xl p-6">
              ...
              {item.student_name && (
                <h3 className="text-lg font-semibold text-brand-text mb-1">{item.student_name}</h3>
              )}
              ...
              {item.caption && (
                <p className="text-sm text-brand-text/70 mb-4 line-clamp-2">{item.caption}</p>
              )}
        ...
        {items.length === 0 && (
          <div className="text-center py-12 text-brand-text/70">
            <MessageSquare className="h-12 w-12 mx-auto mb-4 text-brand-text/60" />
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm">
          <div className="bg-[var(--card)] brightness-95 border border-brand-border rounded-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto">
            ...
            <form onSubmit={handleSave} className="p-6 space-y-4">
              <div>
                <label className="block text-sm font-medium text-brand-text/70 mb-2">Student Name (Optional)</label>
                <input
                  type="text"
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-medium text-brand-text/70 mb-2">Cohort (Optional)</label>
                  <input
                    type="text"
                ...
                <div>
                  <label className="block text-sm font-medium text-brand-text/70 mb-2">Course (Optional)</label>
                  <input
                    type="text"
                ...
                <div className="flex gap-4">
                  <label className="flex items-center gap-2 cursor-pointer">
                    <input
                      ...
                      className="w-4 h-4 border-brand-border bg-[var(--card)] brightness-95 text-[#10b981] focus:ring-[#00f0ff]"
                    />
                    <span className="text-sm text-brand-text/70">Image</span>
                  </label>
                  <label className="flex items-center gap-2 cursor-pointer">
                    <input
                      type="radio"
                      ...
                      className="w-4 h-4 border-brand-border bg-[var(--card)] brightness-95 text-[#10b981] focus:ring-[#00f0ff]"
                    />
                    <span className="text-sm text-brand-text/70">Video</span>
                  </label>
                </div>
              </div>
              <div>
                ...
                <div className="space-y-3">
                  ...
                  {formData.mediaFile && (
                    <div className="flex items-center justify-between p-2 bg-[var(--card)] brightness-95 border border-brand-border rounded">
                      <span className="text-xs text-brand-text/70 truncate">{formData.mediaFile.name}</span>
                      <button
                        type="button"
              ...
              {formData.mediaType === 'video' && (
                <div>
                  <label className="block text-sm font-medium text-brand-text/70 mb-2">Thumbnail/Poster (Optional)</label>
                  <div className="space-y-3">
                    <label className="flex items-center gap-2 px-4 py-3 bg-[var(--card)] brightness-95 border border-brand-border rounded-lg cursor-pointer hover:border-[#10b981] transition-colors">
                      <Upload className="h-4 w-4 text-brand-text/70" />
                      <span className="text-sm text-brand-text/70">
                    ...
                    {formData.thumbnailFile && (
                      <div className="flex items-center justify-between p-2 bg-[var(--card)] brightness-95 border border-brand-border rounded">
                        <span className="text-xs text-brand-text/70 truncate">{formData.thumbnailFile.name}</span>
                        <button
                          type="button"
              ...
              <div>
                <label className="block text-sm font-medium text-brand-text/70 mb-2">Profile Picture (Optional)</label>
                <div className="space-y-3">
                  <label className="flex items-center gap-2 px-4 py-3 bg-[var(--card)] brightness-95 border border-brand-border rounded-lg cursor-pointer hover:border-[#10b981] transition-colors">
                    <Upload className="h-4 w-4 text-brand-text/70" />
                    <span className="text-sm text-brand-text/70">
                  ...
                  {formData.profileImageFile && (
                    <div className="flex items-center justify-between p-2 bg-[var(--card)] brightness-95 border border-brand-border rounded">
                      <span className="text-xs text-brand-text/70 truncate">{formData.profileImageFile.name}</span>
                      <button
                        type="button"
              ...
              <div>
                <label className="block text-sm font-medium text-brand-text/70 mb-2">Social Profile URL (Optional)</label>
                <input
                  type="url"
                  ...
                  className="w-full px-4 py-2 bg-[var(--card)] brightness-95 border border-brand-border rounded-lg text-sm text-brand-text focus:outline-none focus:border-[#10b981]"
                />
                <p className="text-xs text-brand-text/60 mt-1">Must start with http:// or https://</p>
              </div>
              <div>
                <label className="block text-sm font-medium text-brand-text/70 mb-2">Caption (Optional)</label>
                <textarea
              ...
              <div className="flex items-center gap-4">
                <label className="flex items-center gap-2 cursor-pointer">
                  ...
                  />
                  <span className="text-sm text-brand-text/70">Featured</span>
                </label>
                <label className="flex items-center gap-2 cursor-pointer">
                  <input
                    type="checkbox"
