        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-4">
              ...
              <div>
                <h1 className="text-xl font-bold text-brand-text">Announcements</h1>
                <p className="text-sm text-brand-text/70">Manage website announcements</p>
              </div>
            </div>
            <button
              onClick={() => {
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {items.map((item) => (
            <div key={item.id} className="border border-brand-border bg-[var(--card)] brightness-95/50 backdrop-blur-xl rounded-xl p-6">
              ...
              <h3 className="text-lg font-semibold text-brand-text mb-2 line-clamp-2">{item.text}</h3>
              {item.cta_text && (
                <p className="text-sm text-brand-text/70 mb-4">CTA: {item.cta_text}</p>
              )}
              <div className="space-y-2 mb-4">
                <p className="text-xs text-brand-text/60">Position: {item.display_position}</p>
        ...
        {items.length === 0 && (
          <div className="text-center py-12 text-brand-text/70">
            <FileText className="h-12 w-12 mx-auto mb-4 text-brand-text/60" />
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm">
          <div className="bg-[var(--card)] brightness-95 border border-brand-border rounded-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto">
            ...
            <form onSubmit={handleSave} className="p-6 space-y-4">
              <div>
                <label className="block text-sm font-medium text-brand-text/70 mb-2">Announcement Text *</label>
                <textarea
                  value={formData.text}
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-medium text-brand-text/70 mb-2">CTA Text</label>
                  <input
                    type="text"
                ...
                <div>
                  <label className="block text-sm font-medium text-brand-text/70 mb-2">CTA Link</label>
                  <input
                    type="url"
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-medium text-brand-text/70 mb-2">Display Position</label>
                  <select
                ...
                <div>
                  <label className="block text-sm font-medium text-brand-text/70 mb-2">Display Type</label>
                  <select
              ...
              <div className="flex gap-3 pt-4 border-t border-brand-border">
                <button
                  type="submit"
                  className="flex-1 flex items-center justify-center gap-2 px-4 py-3 bg-brand-bg text-[#00363a] rounded-lg font-medium hover:bg-brand-bg/90 transition-colors"
                >
