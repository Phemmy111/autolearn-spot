    ...
    <div className="min-h-screen pb-12 text-brand-text font-sans">
      ...
      <header className="h-16 bg-[var(--card)] brightness-95 border-b border-brand-border px-6 sm:px-8 flex items-center justify-between sticky top-0 z-30">
        <div className="flex items-center gap-2 text-sm text-brand-text/60 font-medium">
          <span className="hover:text-brand-text cursor-pointer transition-colors">Admin</span>
      ...
      <main className="px-6 sm:px-8 pt-8 max-w-[1600px] mx-auto">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
          <div>
            <h1 className="text-3xl font-extrabold text-brand-text tracking-tight">Authors</h1>
          ...
          <button className="inline-flex items-center justify-center gap-2 px-5 py-2.5 bg-blue-600 text-brand-text font-bold text-sm rounded-lg hover:bg-blue-700 transition-colors shadow-sm whitespace-nowrap">
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-5 gap-4 sm:gap-6 mb-8">
          {[
            ...
            <div key={i} className="bg-[var(--card)] brightness-95 rounded-2xl p-5 border border-gray-100 shadow-sm hover:shadow-md transition-shadow">
              ...
              <h3 className="text-3xl font-bold text-brand-text tracking-tight">{stat.value}</h3>
              <div className="flex items-center justify-between mt-1">
                <p className="text-sm font-semibold text-brand-text/60">{stat.label}</p>
              </div>
        ...
        <div className="bg-[var(--card)] brightness-95 border border-brand-border rounded-2xl shadow-sm overflow-hidden">
                ...
                {['All Authors', 'Active', 'Pending', 'Suspended', 'Rejected'].map((tab) => (
                  <button
                    ...
                    className={`pb-4 text-sm font-bold whitespace-nowrap transition-colors relative ${
                      activeTab === tab ? 'text-blue-600' : 'text-brand-text/60 hover:text-brand-text'
                    }`}
                  >
                    {tab}
                    {activeTab === tab && (
          ...
          <div className="p-5 flex flex-col sm:flex-row gap-4 justify-between items-center bg-brand-bg/50 border-b border-gray-100">
            <div className="relative w-full sm:w-80">
              <Search className="h-4 w-4 absolute left-3 top-1/2 -translate-y-1/2 text-gray-400" />
              <input 
                type="text" 
                placeholder="Search authors..." 
                className="w-full pl-9 pr-4 py-2 bg-[var(--card)] brightness-95 border border-brand-border rounded-lg text-sm font-medium focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 shadow-sm transition-all"
              />
            </div>
            <button className="w-full sm:w-auto inline-flex items-center justify-center gap-2 px-4 py-2 bg-[var(--card)] brightness-95 border border-brand-border text-gray-700 font-bold text-sm rounded-lg hover:bg-brand-bg shadow-sm transition-colors whitespace-nowrap">
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-brand-bg/80 border-b border-brand-border">
                  <th className="px-6 py-4 text-xs font-bold text-brand-text/60 uppercase tracking-wider">Author</th>
                  <th className="px-6 py-4 text-xs font-bold text-brand-text/60 uppercase tracking-wider">Skills</th>
                  <th className="px-6 py-4 text-xs font-bold text-brand-text/60 uppercase tracking-wider text-center">Products</th>
                  <th className="px-6 py-4 text-xs font-bold text-brand-text/60 uppercase tracking-wider text-right">Students</th>
                  <th className="px-6 py-4 text-xs font-bold text-brand-text/60 uppercase tracking-wider text-right">Revenue</th>
                  <th className="px-6 py-4 text-xs font-bold text-brand-text/60 uppercase tracking-wider">Status</th>
                  <th className="px-6 py-4 text-xs font-bold text-brand-text/60 uppercase tracking-wider">Joined</th>
                  <th className="px-6 py-4 text-xs font-bold text-brand-text/60 uppercase tracking-wider text-right">Actions</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-gray-100">
                {authors.map((author) => (
                  <tr key={author.id} className="hover:bg-brand-bg/50 transition-colors group cursor-pointer" onClick={() => router.push(`/admin/authors/applications/${author.id}`)}>
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <div className="font-bold text-brand-text group-hover:text-blue-600 transition-colors">{author.name}</div>
                          <div className="text-xs font-medium text-brand-text/60 mt-0.5">{author.expertise}</div>
                        </div>
                      </div>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="flex gap-2">
                        {author.skills.map(skill => (
                          <span key={skill} className="px-2 py-1 bg-[var(--card)] brightness-95 text-brand-text/70 border border-brand-border rounded-md text-[11px] font-bold tracking-wide">
                            {skill}
                          </span>
                        ))}
                      </div>
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-right">
                      <span className="font-bold text-brand-text">{author.revenue}</span>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <span className="text-sm font-medium text-brand-text/60">{author.joined}</span>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap text-right" onClick={(e) => e.stopPropagation()}>
                      <button className="p-1.5 text-gray-400 hover:text-brand-text/70 hover:bg-[var(--card)] brightness-95 rounded-lg transition-colors">
