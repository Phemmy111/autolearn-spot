...
export default function RCTestingDashboard() {
  ...
  const getStatusIcon = (status: string) => {
    switch (status) {
      ...
      case 'skipped': return <Clock className="h-5 w-5 text-brand-text/60" />
    ...
    <div className="min-h-screen bg-brand-bg">
      <div className="container mx-auto px-4 py-12 max-w-7xl">
        <div className="mb-8">
          ...
          <h1 className="font-heading text-4xl font-bold text-brand-text mb-4">RC Testing Dashboard</h1>
          <p className="font-mono text-sm text-brand-text/70">Execute regression tests and generate RC report</p>
        </div>

        {!sessionStarted ? (
          <Card className="bg-[var(--card)] brightness-95 border-brand-border">
            <CardHeader>
              <CardTitle className="text-brand-text">Start RC Testing Session</CardTitle>
              <CardDescription className="text-brand-text/70">Enter your name to begin testing</CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
              <div className="space-y-2">
                <Label htmlFor="executedBy" className="text-brand-text">Tester Name</Label>
                <Input
                  id="executedBy"
              ...
              <Button
                ...
                className="w-full bg-brand-bg text-black hover:bg-[var(--card)] brightness-95"
              >
          ...
          <Tabs defaultValue="test-cases" className="space-y-4">
            <TabsList className="bg-brand-bg border-brand-border">
              <TabsTrigger value="test-cases" className="data-[state=active]:bg-brand-bg data-[state=active]:text-black">Test Cases</TabsTrigger>
              <TabsTrigger value="report" className="data-[state=active]:bg-brand-bg data-[state=active]:text-black">Report</TabsTrigger>
            </TabsList>

            <TabsContent value="test-cases" className="space-y-4">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-brand-text font-medium">Testing Session</p>
                  <p className="text-sm text-brand-text/70">Tester: {executedBy}</p>
                </div>
                <Button
                  onClick={() => {
                    setSessionStarted(false)
                  ...
                  variant="outline"
                  className="border-brand-border text-brand-text/70 hover:bg-brand-bg"
                >
              ...
              <div className="grid gap-4 md:grid-cols-2">
                {rcTestCases.map((test) => {
                  ...
                  const statusColor = result?.status === 'pass' ? 'text-green-400' :
                                     ...
                                     result?.status === 'skipped' ? 'text-yellow-400' : 'text-brand-text/70'
                  return (
                    <Card
                      key={test.id}
                      className={`bg-[var(--card)] brightness-95 border-brand-border cursor-pointer hover:border-[#10b981]/30 transition-all ${selectedTest === test.id ? 'border-[#10b981]' : ''}`}
                      onClick={() => setSelectedTest(test.id)}
                    >
                      <CardHeader className="pb-3">
                        <div className="flex items-start justify-between">
                          <div className="flex-1">
                            <CardTitle className="text-brand-text text-base mb-1">{test.name}</CardTitle>
                            <CardDescription className="text-brand-text/70 text-xs">{test.category}</CardDescription>
                          </div>
                      ...
                      <CardContent>
                        <div className="flex items-center justify-between">
                          ...
                          <Badge variant="outline" className="border-brand-border text-brand-text/70">
                            {test.severity}
                          </Badge>
                        </div>
                      </CardContent>
              ...
              {selectedTest && (
                <Card className="bg-[var(--card)] brightness-95 border-brand-border">
                  <CardHeader>
                    <CardTitle className="text-brand-text">Test Execution</CardTitle>
                  ...
                  <CardContent className="space-y-4">
                    <div className="space-y-2">
                      ...
                      <div className="flex gap-2">
                        <Button
                          ...
                          className={testResults.get(selectedTest)?.status === 'pass' ? 'bg-green-500 text-brand-text' : 'border-brand-border text-brand-text/70'}
                        >
                        ...
                        <Button
                          ...
                          className={testResults.get(selectedTest)?.status === 'fail' ? 'bg-red-500 text-brand-text' : 'border-brand-border text-brand-text/70'}
                        >
                        ...
                        <Button
                          ...
                          className={testResults.get(selectedTest)?.status === 'skipped' ? 'bg-yellow-500 text-brand-text' : 'border-brand-border text-brand-text/70'}
                        >
                    ...
                    <div className="space-y-2">
                      <Label className="text-brand-text">Actual Result</Label>
                      <Textarea
                        ...
                        className="bg-brand-bg border-brand-border text-brand-text min-h-[100px]"
                      />
                    </div>

                    <div className="space-y-2">
                      <Label className="text-brand-text">Notes</Label>
                      <Textarea
                        placeholder="Additional notes or observations"
                        value={testResults.get(selectedTest)?.notes || ''}
                        onChange={(e) => updateTestResult(selectedTest, { notes: e.target.value })}
                        className="bg-brand-bg border-brand-border text-brand-text min-h-[80px]"
                      />
                    </div>

                    {testResults.get(selectedTest)?.status === 'fail' && (
                      <div className="space-y-2">
                        <Label className="text-brand-text">Severity</Label>
                        <div className="flex gap-2">
                          <Button
                            ...
                            className={testResults.get(selectedTest)?.severity === 'critical' ? 'bg-red-600 text-brand-text' : 'border-brand-border text-brand-text/70'}
                            size="sm"
                          >
                            Critical
                          </Button>
                          <Button
                            ...
                            className={testResults.get(selectedTest)?.severity === 'high' ? 'bg-orange-500 text-brand-text' : 'border-brand-border text-brand-text/70'}
                            size="sm"
                          >
                            High
                          </Button>
                          <Button
                            ...
                            className={testResults.get(selectedTest)?.severity === 'medium' ? 'bg-yellow-500 text-brand-text' : 'border-brand-border text-brand-text/70'}
                            size="sm"
                          >
                            Medium
                          </Button>
                          <Button
                            ...
                            className={testResults.get(selectedTest)?.severity === 'low' ? 'bg-gray-500 text-brand-text' : 'border-brand-border text-brand-text/70'}
                            size="sm"
                          >
                            Low
                          </Button>
                    ...
                    <Button
                      ...
                      variant="outline"
                      className="w-full border-brand-border text-brand-text/70 hover:bg-brand-bg"
                    >
                      Close Test Details
                    </Button>
                  </CardContent>
            ...
            <TabsContent value="report" className="space-y-4">
              <Card className="bg-[var(--card)] brightness-95 border-brand-border">
                <CardHeader>
                  <CardTitle className="text-brand-text">RC Report</CardTitle>
                  <CardDescription className="text-brand-text/70">Summary of test execution results</CardDescription>
                </CardHeader>
                <CardContent className="space-y-4">
                  {!report ? (
                    <Button
                    ...
                    <div className="space-y-4">
                      <div className="grid gap-4 md:grid-cols-4">
                        <div className="space-y-1">
                          <p className="text-sm text-brand-text/70">Total Tests</p>
                          <p className="text-2xl font-bold text-brand-text">{report.totalTests}</p>
                        </div>
                        <div className="space-y-1">
                          <p className="text-sm text-brand-text/70">Passed</p>
                          <p className="text-2xl font-bold text-green-400">{report.passed}</p>
                        </div>
                        <div className="space-y-1">
                          <p className="text-sm text-brand-text/70">Failed</p>
                          <p className="text-2xl font-bold text-red-400">{report.failed}</p>
                        </div>
                        <div className="space-y-1">
                          <p className="text-sm text-brand-text/70">Skipped</p>
                          <p className="text-2xl font-bold text-yellow-400">{report.skipped}</p>
                        </div>
                      </div>

                      <div className="border-t border-brand-border pt-4">
                        <h3 className="text-brand-text font-medium mb-2">Failure Breakdown</h3>
                        <div className="grid gap-2 md:grid-cols-4">
                          <div className="space-y-1">
                            <p className="text-sm text-brand-text/70">Critical</p>
                            <p className="text-xl font-bold text-red-600">{report.criticalFailures}</p>
                          </div>
                          <div className="space-y-1">
                            <p className="text-sm text-brand-text/70">High</p>
                            <p className="text-xl font-bold text-orange-500">{report.highFailures}</p>
                          </div>
                          <div className="space-y-1">
                            <p className="text-sm text-brand-text/70">Medium</p>
                            <p className="text-xl font-bold text-yellow-500">{report.mediumFailures}</p>
                          </div>
                          <div className="space-y-1">
                            <p className="text-sm text-brand-text/70">Low</p>
                      ...
                      <div className="border-t border-brand-border pt-4">
                        <h3 className="text-brand-text font-medium mb-2">Recommendation</h3>
                        <Badge className={report.recommendation === 'GO' ? 'bg-green-500 text-brand-text' : 'bg-red-500 text-brand-text'}>
                      ...
                      {report.blockingIssues.length > 0 && (
                        <div className="border-t border-brand-border pt-4">
                          <h3 className="text-brand-text font-medium mb-2">Blocking Issues</h3>
