  ...
  return (
    <div className="min-h-screen bg-[var(--card)] brightness-95]">
      <div className="container mx-auto px-4 py-12 max-w-4xl">
        <Link
          href="/admin/results"
          className="flex items-center gap-2 text-brand-text/70 hover:text-brand-text font-mono text-sm mb-8 transition-colors"
        >
        ...
        {/* Header */}
        <div className="border border-brand-border bg-[var(--card)] brightness-95 rounded-xl p-8 mb-8">
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
            <div>
              <div className="flex items-center gap-3 mb-2">
                <User className="h-6 w-6 text-[#10b981]" />
                <h1 className="font-heading text-2xl font-bold text-brand-text">{response.user_name}</h1>
              </div>
              <p className="font-mono text-sm text-brand-text/70 mb-4">{response.user_email}</p>
              
              <div className="flex items-center gap-4 font-mono text-xs text-brand-text/60">
            ...
            <div className="flex items-center gap-8">
              <div className="text-center">
                <p className="font-mono text-xs text-brand-text/60 uppercase tracking-wider mb-1">Score</p>
                <p className="font-mono text-2xl font-bold text-[#10b981]">
                  {response.score}<span className="text-brand-text/60 text-sm">/{response.total_points}</span>
                </p>
              </div>
              <div className="text-center">
                <p className="font-mono text-xs text-brand-text/60 uppercase tracking-wider mb-1">Status</p>
                {response.passed ? (
        ...
        {/* Detailed Review */}
        <h2 className="font-heading text-xl font-bold text-brand-text mb-6">Detailed Review</h2>
              ...
              >
                <div className="flex items-start gap-4 mb-4">
                  ...
                  <div className="flex-1">
                    <p className="font-heading text-brand-text mb-3 leading-relaxed">
                      <span className="text-brand-text/60 font-mono mr-2 text-sm">Q{i + 1}.</span>
                      {q.question_text}
                    </p>
                    <div className="space-y-2 font-mono text-sm">
                      <div className="flex items-start gap-2">
                        <span className="text-brand-text/60 min-w-[120px]">Student Answer:</span>
                      ...
                      {!isCorrect && (
                        <div className="flex items-start gap-2">
                          <span className="text-brand-text/60 min-w-[120px]">Correct Answer:</span>
                ...
                {q.explanation && (
                  <div className="mt-4 pt-4 border-t border-brand-border/50">
                    <p className="font-mono text-xs uppercase tracking-wider text-brand-text/70 mb-2">Explanation</p>
