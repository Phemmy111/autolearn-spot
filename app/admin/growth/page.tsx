    ...
    <div className="space-y-6 max-w-7xl mx-auto">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          ...
          <p className="text-neutral-600 text-sm">Manage ambassadors, partners, and payouts.</p>
        </div>
      </div>
      ...
      {!loading && activeTab === 'overview' && stats && (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          <div className="bg-neutral-50 border border-neutral-200 p-6 rounded-2xl">
            <h3 className="text-sm text-neutral-600 mb-1">Student Ambassadors</h3>
            <p className="text-3xl font-bold">{stats.studentAmbassadors}</p>
          </div>
          <div className="bg-neutral-50 border border-neutral-200 p-6 rounded-2xl">
            <h3 className="text-sm text-neutral-600 mb-1">Community Ambassadors</h3>
          ...
          <table className="w-full text-left text-sm">
            <thead className="bg-gray-50/50 border-b border-neutral-200">
              <tr>
                ...
                <th className="px-6 py-4 font-medium text-neutral-600">Promotion Method</th>
            ...
            <tbody className="divide-y divide-[#1f2229]">
              ...
              {applications.length === 0 && (
                <tr><td colSpan={5} className="px-6 py-12 text-center text-neutral-600">No applications found.</td></tr>
              )}
            </tbody>
          </table>
        </div>
      ...
      {!loading && activeTab === 'partners' && (
        <div className="grid lg:grid-cols-3 gap-6">
          <div className="lg:col-span-2 bg-neutral-50 border border-neutral-200 rounded-2xl overflow-hidden">
            ...
            <table className="w-full text-left text-sm">
              <thead className="bg-gray-50/50 border-b border-neutral-200">
                <tr>
                  ...
                  <th className="px-6 py-4 font-medium text-neutral-600">Rate</th>
                  <th className="px-6 py-4 font-medium text-neutral-600">Status</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-[#1f2229]">
                {partners.map(p => (
                  <tr key={p.id}>
                    <td className="px-6 py-4">
                      ...
                      <p className="text-neutral-600 text-xs">{p.email}</p>
                    </td>
                    <td className="px-6 py-4 text-neutral-600">{p.platform}</td>
                ...
                {partners.length === 0 && (
                  <tr><td colSpan={4} className="px-6 py-8 text-center text-neutral-600">No influencer partners found.</td></tr>
                )}
              </tbody>
            </table>
          </div>
          ...
          <div className="bg-neutral-50 border border-neutral-200 rounded-2xl p-6 h-fit">
            ...
            <form onSubmit={handleCreateInfluencer} className="space-y-4 text-sm">
              ...
              <button disabled={isSubmitting} type="submit" className="w-full py-2.5 rounded-lg bg-gray-50 text-black font-bold hover:bg-gray-100 transition-colors disabled:opacity-50">
            ...
            <tbody className="divide-y divide-[#1f2229]">
              {withdrawals.map(w => (
                <tr key={w.id} className="hover:bg-gray-100/5 transition-colors">
                  ...
                  <td className="px-6 py-4">
                    <p className="font-medium text-neutral-900">{w.bank_name}</p>
                    <p className="text-neutral-600 text-xs">{w.account_number} ({w.account_name})</p>
                  </td>
                  <td className="px-6 py-4 font-medium text-neutral-900">₦{w.amount.toLocaleString()}</td>
                  <td className="px-6 py-4">
                    <span className={`px-2 py-1 rounded text-xs font-medium ${
              ...
              {withdrawals.length === 0 && (
                <tr><td colSpan={6} className="px-6 py-12 text-center text-neutral-600">No withdrawals found.</td></tr>
              )}
            </tbody>
          </table>
        </div>
