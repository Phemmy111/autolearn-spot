    ...
    <div className="space-y-6 max-w-7xl mx-auto">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          ...
          <p className="text-brand-text/70 text-sm">Manage ambassadors, partners, and payouts.</p>
        </div>
      </div>
      ...
      {loading && <div className="py-12 flex justify-center"><Loader2 className="h-8 w-8 animate-spin text-[#10b981]" /></div>}

      {!loading && activeTab === 'overview' && stats && (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          <div className="bg-brand-bg border border-brand-border p-6 rounded-2xl">
            <h3 className="text-sm text-brand-text/70 mb-1">Student Ambassadors</h3>
            <p className="text-3xl font-bold">{stats.studentAmbassadors}</p>
          </div>
          <div className="bg-brand-bg border border-brand-border p-6 rounded-2xl">
            <h3 className="text-sm text-brand-text/70 mb-1">Community Ambassadors</h3>
          ...
          <table className="w-full text-left text-sm">
            <thead className="bg-brand-bg/50 border-b border-brand-border">
              <tr>
                ...
                <th className="px-6 py-4 font-medium text-brand-text/70">Promotion Method</th>
            ...
            <tbody className="divide-y divide-[#1f2229]">
              {applications.map(app => (
                <tr key={app.id} className="hover:bg-[var(--card)] brightness-95/5 transition-colors">
                  <td className="px-6 py-4">
                    ...
                    <p className="text-brand-text/70 text-xs">{app.email}</p>
                  </td>
                  <td className="px-6 py-4">
                    <span className="px-2 py-1 rounded bg-[var(--card)] brightness-95/5 text-xs">{app.is_student ? 'Student' : 'Community'}</span>
                  </td>
                  <td className="px-6 py-4 text-brand-text/70 truncate max-w-[200px]" title={app.promotion_method}>
              ...
              {applications.length === 0 && (
                <tr><td colSpan={5} className="px-6 py-12 text-center text-brand-text/70">No applications found.</td></tr>
              )}
            </tbody>
          </table>
        </div>
      ...
      {!loading && activeTab === 'partners' && (
        <div className="grid lg:grid-cols-3 gap-6">
          <div className="lg:col-span-2 bg-brand-bg border border-brand-border rounded-2xl overflow-hidden">
            ...
            <table className="w-full text-left text-sm">
              <thead className="bg-brand-bg/50 border-b border-brand-border">
                <tr>
                  ...
                  <th className="px-6 py-4 font-medium text-brand-text/70">Platform</th>
                  <th className="px-6 py-4 font-medium text-brand-text/70">Rate</th>
                  <th className="px-6 py-4 font-medium text-brand-text/70">Status</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-[#1f2229]">
                {partners.map(p => (
                  <tr key={p.id}>
                    <td className="px-6 py-4">
                      <p className="font-medium text-brand-text">{p.full_name}</p>
                      <p className="text-brand-text/70 text-xs">{p.email}</p>
                    </td>
                    <td className="px-6 py-4 text-brand-text/70">{p.platform}</td>
                ...
                {partners.length === 0 && (
                  <tr><td colSpan={4} className="px-6 py-8 text-center text-brand-text/70">No influencer partners found.</td></tr>
                )}
              </tbody>
            </table>
          </div>

          <div className="bg-brand-bg border border-brand-border rounded-2xl p-6 h-fit">
            <h3 className="font-bold mb-6">Invite Influencer</h3>
            <form onSubmit={handleCreateInfluencer} className="space-y-4 text-sm">
              <input required value={newInfluencer.full_name} onChange={e => setNewInfluencer({...newInfluencer, full_name: e.target.value})} className="w-full bg-[var(--card)] brightness-95 border border-brand-border rounded-lg px-4 py-2 focus:border-[#10b981] outline-none" placeholder="Full Name" />
              <input required type="email" value={newInfluencer.email} onChange={e => setNewInfluencer({...newInfluencer, email: e.target.value})} className="w-full bg-[var(--card)] brightness-95 border border-brand-border rounded-lg px-4 py-2 focus:border-[#10b981] outline-none" placeholder="Email Address" />
              <input value={newInfluencer.phone} onChange={e => setNewInfluencer({...newInfluencer, phone: e.target.value})} className="w-full bg-[var(--card)] brightness-95 border border-brand-border rounded-lg px-4 py-2 focus:border-[#10b981] outline-none" placeholder="Phone Number" />
              <input required value={newInfluencer.platform} onChange={e => setNewInfluencer({...newInfluencer, platform: e.target.value})} className="w-full bg-[var(--card)] brightness-95 border border-brand-border rounded-lg px-4 py-2 focus:border-[#10b981] outline-none" placeholder="Platform (e.g. YouTube)" />
              <input required type="number" value={newInfluencer.commission_rate} onChange={e => setNewInfluencer({...newInfluencer, commission_rate: e.target.value})} className="w-full bg-[var(--card)] brightness-95 border border-brand-border rounded-lg px-4 py-2 focus:border-[#10b981] outline-none" placeholder="Commission Rate (e.g. 2000)" />
              <button disabled={isSubmitting} type="submit" className="w-full py-2.5 rounded-lg bg-brand-bg text-black font-bold hover:bg-[var(--card)] brightness-95 transition-colors disabled:opacity-50">
      ...
      {!loading && activeTab === 'withdrawals' && (
        <div className="bg-brand-bg border border-brand-border rounded-2xl overflow-hidden">
          <table className="w-full text-left text-sm">
            <thead className="bg-brand-bg/50 border-b border-brand-border">
              <tr>
                <th className="px-6 py-4 font-medium text-brand-text/70">Date</th>
                <th className="px-6 py-4 font-medium text-brand-text/70">Type</th>
            ...
            <tbody className="divide-y divide-[#1f2229]">
              {withdrawals.map(w => (
                <tr key={w.id} className="hover:bg-[var(--card)] brightness-95/5 transition-colors">
                  <td className="px-6 py-4 text-brand-text/70">{new Date(w.created_at).toLocaleDateString()}</td>
                  <td className="px-6 py-4 capitalize">{w.user_type}</td>
                  <td className="px-6 py-4">
                    <p className="font-medium text-brand-text">{w.bank_name}</p>
                    <p className="text-brand-text/70 text-xs">{w.account_number} ({w.account_name})</p>
                  </td>
                  <td className="px-6 py-4 font-medium text-brand-text">₦{w.amount.toLocaleString()}</td>
                  <td className="px-6 py-4">
                    <span className={`px-2 py-1 rounded text-xs font-medium ${
              ...
              {withdrawals.length === 0 && (
                <tr><td colSpan={6} className="px-6 py-12 text-center text-brand-text/70">No withdrawals found.</td></tr>
              )}
            </tbody>
          </table>
        </div>
