...
export function AnimatedFeaturesSection() {
  ...
  return (
    <section ref={sectionRef} id="tools" className="py-6 sm:py-8 lg:py-12 bg-brand-bg">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-4 lg:gap-6">
          {featureIcons.map((feature, index) => {
            ...
            return (
              <div
                key={feature.title}
                className={`border border-brand-border bg-brand-bg/80 backdrop-blur-xl rounded-2xl p-4 sm:p-5 lg:p-6 hover:border-[#10b981]/50 transition-all duration-300 hover:shadow-[0_0_20px_rgba(0,240,255,0.1)] card-enhanced reveal-on-scroll ${isVisible ? 'is-visible' : ''}`}
                style={{ transitionDelay: `${0.1 + index * 0.1}s` }}
              >
                <div className="flex h-10 w-10 sm:h-12 sm:w-12 items-center justify-center border border-[#10b981]/60 bg-brand-bg/10 rounded-xl mb-3 sm:mb-4">
                ...
                <p className="text-sm text-brand-text/60">{feature.description}</p>
              </div>
            )
          })}
        </div>
