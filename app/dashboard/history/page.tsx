    ...
    if (entry.type === 'quiz') {
      return (
        <div className="flex flex-col">
          <span className="font-bold text-[#10b981]">{entry.score} pts</span>
      ...
      return entry.ai_score !== null ? (
        <div className="flex flex-col">
          <span className="font-bold text-[#10b981]">{entry.ai_score} pts</span>
          ...
          <table className="w-full text-left text-sm font-mono text-[#b9cacb]">
            ...
            <tbody className="divide-y divide-[#1f2229]">
              {history.map((entry, idx) => (
                <tr key={entry.id} className={`hover:bg-[#c0c4c9] transition-colors ${idx % 2 === 0 ? 'bg-[#c0c4c9]' : 'bg-[#c0c4c9]'}`}>
