...
export default function HistoryPage() {
  ...
  return (
    <div className="mx-auto max-w-4xl px-4 py-12 sm:px-6">
      ...
      <div className="overflow-hidden rounded-xl border border-border bg-[var(--card)] shadow-2xl">
          ...
          <table className="w-full text-left text-sm font-mono text-[var(--muted-foreground)]">
            ...
            <tbody className="divide-y divide-[var(--border)]">
              {history.map((entry, idx) => (
                <tr key={entry.id} className={`hover:bg-[var(--surface-hover)] transition-colors ${idx % 2 === 0 ? 'bg-[var(--card)]' : 'bg-[var(--surface-alt)]'}`}>
