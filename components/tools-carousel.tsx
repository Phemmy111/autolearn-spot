  ...
  return (
    <div className="page-animate">
      <div className="tools-coverflow-stage relative mx-auto h-[440px] max-w-6xl overflow-hidden sm:h-[520px]">
        <div className="pointer-events-none absolute inset-x-0 bottom-8 mx-auto h-px max-w-3xl bg-gray-100]" />
        {tools.map((tool, index) => {
          ...
          return (
            <article
              ...
              className={`absolute left-1/2 top-1/2 flex h-[360px] w-[250px] flex-col items-center justify-between border bg-gray-100]/95 p-7 text-center shadow-[0_28px_80px_rgba(0,0,0,0.35)] transition-all duration-500 ease-out sm:h-[440px] sm:w-[320px] ${
                isActive
            ...
            >
              <div className="flex w-full items-center justify-between font-mono text-[10px] uppercase tracking-[0.12em] text-[#5d5f63]">
                ...
                <span className={`h-2 w-2 ${isActive ? 'bg-gray-100]' : 'bg-gray-100]'}`} />
              </div>
              <div>
                <div className="mx-auto flex h-24 w-24 items-center justify-center border border-[#10b981]/70 bg-gray-100]/10 text-[#10b981] sm:h-28 sm:w-28">
              ...
              <button
                className="w-full border border-neutral-200 px-4 py-3 font-mono text-[10px] uppercase tracking-[0.14em] text-[#10b981] transition hover:border-[#10b981]/80 hover:bg-gray-100]/10"
                onClick={(event) => {
        ...
        {tools.map((tool, index) => (
          <button
            ...
            className={`h-1.5 w-8 transition ${
              index === activeIndex ? 'bg-gray-100]' : 'bg-gray-100] hover:bg-gray-100]'
            }`}
            key={tool.title}
            onClick={() => setActiveIndex(index)}
            type="button"
