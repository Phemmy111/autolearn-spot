...
import {
  ArrowLeft, Send, Paperclip, Mic, MicOff, Play, Pause,
  StopCircle, Download, X, FileText, Image as ImageIcon, File
} from "lucide-react";
...
function MessageBubble({ message, isMe, partnerName }: { message: Message; isMe: boolean; partnerName?: string }) {
  ...
  return (
    <div className={`flex ${isMe ? "justify-end" : "justify-start"} mb-2`}>
      <div className={`max-w-[70%] rounded-2xl px-3 py-2 shadow-sm ${isMe ? "bg-sky-600 text-white rounded-tr-sm" : "bg-white text-gray-800 rounded-tl-sm border border-gray-100"}`}>
        {/* Attachments */}
        {atts.map((att) => {
          const isImage = att.mime_type?.startsWith("image/");
          const isAudio = att.mime_type?.startsWith("audio/");
...
function MessageInput({ conversationId, onSent }: { conversationId: string; onSent: () => void }) {
  ...
  const removeAttachment = (previewUrl: string) => {
    setPendingAttachments(prev => { const a = prev.find(a => a.previewUrl === previewUrl); if (a) URL.revokeObjectURL(a.previewUrl); return prev.filter(a => a.previewUrl !== previewUrl); });
  };
  ...
  const doSend = async () => {
    ...
    try {
      ...
      if (attachments.length > 0) {
        ...
        else msgType = "IMAGE"; // Use IMAGE as fallback for docs (enum only supports TEXT/IMAGE/VOICE)
      }
  ...
  return (
    <div className="flex flex-col h-screen bg-gray-50">
      ...
      <div className="flex-shrink-0">
        <MessageInput conversationId={conversationId} onSent={fetchMessages} />
      </div>
    </div>
  );
}
