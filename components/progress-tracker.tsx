...
export function ProgressBar({ totalVideos }: { totalVideos: number }) {
  ...
  if (isLoading) {
    return (
      <div className="mb-10 border border-neutral-200 bg-gray-100] p-5 animate-pulse">
        <div className="h-4 w-1/3 bg-gray-100] mb-3"></div>
  ...
  return (
    <div className="mb-10 border border-neutral-200 bg-gray-100] p-5">
      ...
      <div className="relative h-2 w-full overflow-hidden bg-gray-100]">
        <div
          className="absolute inset-y-0 left-0 bg-gray-100] transition-all duration-700 ease-out"
          style={{ width: `${pct}%` }}
        />
        {/* Glow effect */}
        <div
          className="absolute inset-y-0 left-0 bg-gray-100] blur-sm opacity-50 transition-all duration-700 ease-out"
          style={{ width: `${pct}%` }}
        />
      </div>
    </div>
...
export function CompletedBadge({ videoId }: { videoId: string }) {
  ...
  return (
    <div className="absolute top-2 left-2 flex items-center gap-1 rounded bg-emerald-500/90 px-2 py-1 font-mono text-[10px] font-bold uppercase text-neutral-900 backdrop-blur">
