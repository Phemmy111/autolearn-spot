...
export function AutolearnBot({ context = 'landing' }: { context?: 'landing' | 'dashboard' }) {
  ...
  const triggerButton = (
    ...
    >
      {/* Tooltip */}
      <span className="hidden sm:block opacity-0 group-hover:opacity-100 transition-opacity duration-300 bg-brand-bg border border-[#3b494b] text-[#e2e8e2] text-xs font-mono px-3 py-2 rounded-lg shadow-lg whitespace-nowrap mr-2">
        Ask Autolearn Bot 🤖
      </span>
      <span
        className={`relative flex h-14 w-14 items-center justify-center rounded-full bg-gradient-to-br from-[#7c3aed] to-[#a855f7] text-brand-text shadow-[0_4px_20px_rgba(124,58,237,0.4)] transition-all duration-300 hover:scale-110 hover:shadow-[0_6px_28px_rgba(124,58,237,0.55)] ${showPulse ? 'animate-bounce' : ''}`}
      >
  ...
  const chatModal = isOpen && isMounted
    ? createPortal(
        ...
        >
          ...
          {/* Chat window */}
          <div className="relative flex flex-col w-full sm:w-[420px] h-[85vh] sm:h-[600px] max-h-[85vh] rounded-2xl border border-[#3b494b] bg-brand-bg shadow-[0_30px_100px_rgba(124,58,237,0.15)] overflow-hidden">
            {/* Glow accent */}
            ...
            {/* ── Header ── */}
            <div className="flex items-center justify-between border-b border-brand-border bg-brand-bg px-4 py-3">
              <div className="flex items-center gap-3">
                <div className="flex h-10 w-10 items-center justify-center rounded-full bg-gradient-to-br from-[#7c3aed] to-[#a855f7] text-brand-text">
              ...
              <div className="flex items-center gap-1">
                <button
                  ...
                  className="flex h-9 w-9 items-center justify-center rounded-lg text-[#5d5f63] transition hover:bg-brand-bg hover:text-brand-text/60"
                  type="button"
                  ...
                  <div className="w-full space-y-2">
                    {quickPrompts.map((prompt) => (
                      <button
                        ...
                        className="w-full text-left border border-brand-border bg-brand-bg rounded-xl px-4 py-3 text-xs text-brand-text/60 font-mono transition hover:border-[#7c3aed]/50 hover:bg-brand-bg hover:text-[#e2e8e2]"
                        type="button"
                      >
                ...
                >
                  <div
                    className={`max-w-[85%] rounded-2xl px-4 py-3 text-sm leading-relaxed ${
                      msg.role === 'user'
                        ? 'bg-gradient-to-br from-[#7c3aed] to-[#a855f7] text-brand-text rounded-br-md'
                        : 'bg-brand-bg border border-brand-border text-[#e2e8e2] rounded-bl-md'
                    }`}
                  >
                    <p className="whitespace-pre-wrap break-words">{msg.content}</p>
                    <p className={`text-[10px] mt-1.5 ${msg.role === 'user' ? 'text-brand-text/50' : 'text-[#5d5f63]'}`}>
            ...
            {/* ── Input area ── */}
            <div className="border-t border-brand-border bg-brand-bg px-3 py-3">
              <div className="flex items-end gap-2 rounded-xl border border-brand-border bg-brand-bg px-3 py-2 focus-within:border-[#7c3aed]/50 transition-colors">
                <textarea
                  ref={inputRef}
                  value={input}
                ...
                <button
                  ...
                  className={`flex h-9 w-9 shrink-0 items-center justify-center rounded-lg transition-all ${
                    input.trim() && !isLoading
                      ? 'bg-gradient-to-br from-[#7c3aed] to-[#a855f7] text-brand-text hover:opacity-90 shadow-[0_2px_12px_rgba(124,58,237,0.3)]'
                      : 'bg-brand-bg text-[#5d5f63] cursor-not-allowed'
                  }`}
                  type="button"
