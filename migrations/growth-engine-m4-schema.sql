...
-- 2. Ensure Commissions table exists (in case M3 was skipped)
CREATE TABLE IF NOT EXISTS public.commissions (id UUID DEFAULT gen_random_uuid() PRIMARY KEY, referrer_id TEXT NOT NULL, referrer_type TEXT NOT NULL DEFAULT 'student', referee_email TEXT NOT NULL, referral_code VARCHAR(8) NOT NULL, payment_reference TEXT NOT NULL UNIQUE, amount INTEGER NOT NULL DEFAULT 1000, status TEXT NOT NULL DEFAULT 'pending', holding_period_ends_at TIMESTAMP WITH TIME ZONE, reversal_reason TEXT, created_at TIMESTAMP WITH TIME ZONE DEFAULT TIMEZONE('utc'::text, NOW()) NOT NULL, updated_at TIMESTAMP WITH TIME ZONE DEFAULT TIMEZONE('utc'::text, NOW()) NOT NULL);

-- Add withdrawal_id to commissions
ALTER TABLE public.commissions ADD COLUMN IF NOT EXISTS withdrawal_id UUID REFERENCES public.withdrawals(id);

-- 3. Create Withdrawal_Commissions junction table
